...
<div>
  ...
  <a class="navbar-brand" href="#"> <router-link to="/HomePageEmpleado">Productos
      </router-link></a>
  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
    ...
    <div v-if="!hasProduct()">
        <h3> No products...</h3>
        <router-link to="/HomePageEmpleado">Volver a la lista de productos</router-link>
    </div> 
    <div v-else>
      
      <button style="margin:10px;" @click="aprobar()" type="button" class="btn btn-outline-success">Confirmar</button> <!-- Va la parte de factura -->
      
      <button style="margin:10px;" type="button" class="btn btn-outline-warning"><router-link to="/HomePageEmpleado">Editar venta
      </router-link></button>
      <button style="margin:10px;" @click="vaciar()"  type="button" class="btn btn-outline-danger">Cancelar venta</button>
      <div class="card">
  ...
  methods: {
    ...
    aprobar(){
      ...
      if (opcion == true) {
        ...
        this.$router.push({path: "/CrearFactura"})}
        else{
